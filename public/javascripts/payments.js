...
console.log(config);
  ...
  function getExpireDates(strValue){
    let data = strValue.split("/");
  ...
  form
    ...
    let cardNumberEle = form.querySelector("input[name=CardNumber]");
    cardNumberEle.addEventListener('input', (e) => {
      if (!validateLuhnAlgorithm(e.target.value)) {
          e.target.value = e.target.value.replace(/\D/g, "");
          cardNumberEle.classList.add('text-danger');
      } else {
          cardNumberEle.classList.remove("text-danger");
      }
    })
    let expireDateEnput = form.querySelector("input[name=expire]");
    expireDateEnput.addEventListener('keydown', (e) => {
    ...
    expireDateEnput.addEventListener("input", (e) => {
      ...
      if (dateExpression.test(e.target.value)) {
          expireDateEnput.classList.remove("text-danger");
          let data = e.target.value.split("/");
          let expirationMonth = data[0];
          let expirationYear = '20' + data[1];
          // Validate expiration date
          if (!validateExpirationDate(expirationMonth, expirationYear)) {
              expireDateEnput.classList.add("text-danger");
              return "Card has expired";
          } else {
            expireDateEnput.classList.remove("text-danger");
          }
      } else {
        const numInputExp = new RegExp("^([0-9])");
        if(!numInputExp.test(e.target.value)){
          expireDateEnput.value = '';
        }
        expireDateEnput.classList.add("text-danger");
      }
    });
    let cvcNumberEle = form.querySelector("input[name=CVC]");
    cvcNumberEle.addEventListener("input", (e) => {
      if (!validateCVV(e.target.value)) {
          cvcNumberEle.classList.add("text-danger");
          return "Invalid CVV";
      } else {
          cvcNumberEle.classList.remove("text-danger");
      }
  });
    ...
    form.addEventListener('submit', async (event) => {
      ...
      const customer = {
        ...
        country: country,
        zip: form.querySelector('input[name=postal_code]').value,
      }
      const cardInformation = {
        type: 'visa',
        CardNumbers: cardNumberEle.value,
        expMonth: getExpireDates(expireDateEnput.value).expMonth,
        expYear: getExpireDates(expireDateEnput.value).expYear,
        cvc: cvcNumberEle.value
      }
      console.log(event, shipping);
      console.log("card data", cardInformation, customer);
      ...
      const jsonString = response; // Assuming 'paymentIntent' contains the JSON data
     console.log(jsonString);
      const trimmedJsonString = jsonString.trim();
const paymentIntent = JSON.parse(trimmedJsonString);
      
      console.log(response);
      if(response.error){
        var errorAlert = document.getElementById('payment-error');
...
/*
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (payment === 'card') {
      ...
      console.log("card information", payloadSaveDB);
      const response = await stripe.confirmCardPayment(
        paymentIntent.client_secret,
        {
          payment_method: {
...
*/
  ...
  const updateButtonLabel = (paymentMethod, bankName) => {
   ...
   let amount = store.formatPrice(395, activeCurrency);
